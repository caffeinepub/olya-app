...
import type { SpeakerRole, TranscriptEntry } from "../hooks/useDashboardState";
          ...
          {entries.map((entry, index) => {
            ...
            const langDisplay =
              langCode && langCode !== "unknown"
                ? LANG_DISPLAY[langCode] || langCode.toUpperCase()
                : null;
            const showTranslationToggle = shouldShowTranslationToggle(entry);

            return (
              <div
                key={String(entry.timestamp) + String(index)}
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge
                      className={`text-xs ${SPEAKER_COLORS[entry.speaker]}`}
                    >
                      {entry.speaker}
                    </Badge>
                    {langDisplay && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Badge
                            variant="outline"
                            className="text-xs font-mono px-1.5 py-0.5 flex items-center gap-1"
                          >
