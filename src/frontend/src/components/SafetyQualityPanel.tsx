...
function computeMetrics(entries: TranscriptEntry[]) {
  ...
  const ethicalViolations = entries.filter(
    (e) => e.toxicityFlags.length > 0,
  ).length;

  const hallucinationRate = Math.round(
    (ethicalViolations / entries.length) * 100,
  );
  ...
  const trustworthinessScore = Math.max(
    0,
    100 - biaspenalty - violationPenalty,
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-1">
          ...
          <p
            className={`text-2xl font-bold ${hallucinationRate > 20 ? "text-red-500" : hallucinationRate > 10 ? "text-yellow-500" : "text-green-500"}`}
          >
            {hallucinationRate}%
          </p>
        </div>
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-1">
          ...
          <p
            className={`text-2xl font-bold ${ethicalViolations === 0 ? "text-green-500" : ethicalViolations <= 2 ? "text-yellow-500" : "text-red-500"}`}
          >
            {ethicalViolations}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {biasIncidents === 0
            ? "Bias-Free"
            : `${biasIncidents} Bias Alert${biasIncidents > 1 ? "s" : ""}`}
        </Badge>
        <Badge
        ...
        >
          {ethicalViolations === 0
            ? "Ethics OK"
            : `${ethicalViolations} Violation${ethicalViolations > 1 ? "s" : ""}`}
        </Badge>
        <Badge
