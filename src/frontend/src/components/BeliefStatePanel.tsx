...
export default function BeliefStatePanel({
  ...
  useEffect(() => {
    ...
    // Speaker orbit nodes
    const speakers = Object.entries(beliefState.speakerStates) as [
      string,
    ...
    activeSpeakers.forEach(([speaker, state], idx) => {
      const angle =
        (idx / Math.max(activeSpeakers.length, 1)) * Math.PI * 2 - Math.PI / 2;
      const orbitR = 80;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <canvas ref={canvasRef} width={260} height={220} className="rounded-lg" />
