...
import type { BiasCategory, EthicsResult } from "../utils/ethicsSimulator";
    ...
    switch (result.status) {
      ...
      case "Potential Bias": {
        const uniqueCategories = [
          ...new Set(allBiasCategories.map((b) => b.category)),
        ];
        const label =
          ...
          {allBiasCategories.length > 0 && (
            <div className="mt-1">
              ...
              {allBiasCategories.map((bc, i) => (
                <div
                  key={bc.category + String(i)}
                  className="flex items-center gap-1 ml-1"
                >
                  <AlertTriangle size={8} />
                  <span>
                    {BIAS_CATEGORY_LABELS[bc.category] ?? bc.category} [
                    {bc.severity}]
                    {bc.fragment && (
