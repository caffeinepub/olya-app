...
const HALLUCINATION_PATTERNS: Array<{
  ...
  {
    pattern:
      /\b\d{1,3}(\.\d+)?%\s+of\s+(people|users|cases|patients|respondents)\b/i,
    reason: "Specific statistic without source",
    weight: 0.3,
  },
  {
    pattern:
      /\b(exactly|precisely|specifically)\s+\d+\s+(times|cases|instances|people)\b/i,
...
function checkContextContradictions(
  text: string,
  context: string[],
): SuspectPhrase[] {
  ...
  for (const pattern of definitivePatterns) {
    ...
    if (match) {
      ...
      const hasContradiction = context.some((ctx) => {
        ...
        if (matchLower === "always" && ctxLower.includes("sometimes"))
          return true;
        if (matchLower === "never" && ctxLower.includes("sometimes"))
          return true;
        if (matchLower === "everyone" && ctxLower.includes("some people"))
          return true;
        return false;
      });
