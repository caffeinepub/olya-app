...
function assessConversationDirection(
  ...
  for (const e of firstHalf) {
    if (hostileEmotions.includes(e.emotions[0]?.emotionType ?? ""))
      firstHostile++;
  ...
  for (const e of secondHalf) {
    if (hostileEmotions.includes(e.emotions[0]?.emotionType ?? ""))
      secondHostile++;
    if (hostileIntents.includes(e.intents[0]?.intentType ?? ""))
      secondHostile++;
  }
...
export function predictPatterns(
  ...
  const consistency =
    1 -
    (uniqueEmotions + uniqueIntents) /
      (recentEmotions.length + recentIntents.length + 1);
  const confidence = Math.min(
    0.9,
    0.3 + consistency * 0.6 + Math.min(entries.length / 20, 0.2),
  );
