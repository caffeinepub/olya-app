...
function extractPositions(text: string): string[] {
  ...
  if (
    ...
    positions.push("Quality Standard");
  if (
    lower.includes("risk") ||
    lower.includes("liability") ||
    lower.includes("exposure")
  )
    positions.push("Risk Allocation");
  if (
    lower.includes("scope") ||
    lower.includes("deliverable") ||
    lower.includes("feature")
  )
    positions.push("Scope Definition");
  if (
    lower.includes("payment") ||
    lower.includes("invoice") ||
    lower.includes("billing")
  )
    positions.push("Payment Terms");
...
function extractInterests(text: string): string[] {
  ...
  if (
    ...
    interests.push("Profitability");
  if (
    lower.includes("trust") ||
    lower.includes("relationship") ||
    lower.includes("partner")
  )
    interests.push("Long-term Relationship");
  if (
    lower.includes("reputation") ||
    lower.includes("brand") ||
    lower.includes("image")
  )
    interests.push("Reputation");
  if (
    lower.includes("efficiency") ||
    lower.includes("speed") ||
    lower.includes("fast")
  )
    interests.push("Efficiency");
  if (
    lower.includes("security") ||
    lower.includes("stable") ||
    lower.includes("certain")
  )
    interests.push("Stability");
...
export function updateBeliefGraph(
  ...
  for (const pos of positions) {
    ...
    const _edgeId = `${speakerId}_${posId}`;
  ...
  for (const interest of interests) {
    ...
    const _edgeId = `${speakerId}_${intId}`;
  ...
  if (
    ...
    lower.includes("absolutely not")
  )
    persuasionDelta -= 0.05;

  const newTrust = Math.max(
    0.1,
    Math.min(0.95, currentGraph.trustLevel + trustDelta),
  );
  const newPersuasion = Math.max(
    0.1,
    Math.min(0.95, currentGraph.persuasionLevel + persuasionDelta),
  );
  ...
  if (
    ...
    const concernIdx = words.findIndex((w) =>
      ["concern", "worry", "issue", "problem"].includes(w.toLowerCase()),
    );
    if (concernIdx >= 0 && concernIdx < words.length - 1) {
      const concern = words
        .slice(concernIdx + 1, concernIdx + 4)
        .join(" ")
        .replace(/[^a-zA-Z\s]/g, "");
