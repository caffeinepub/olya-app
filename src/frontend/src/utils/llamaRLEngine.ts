...
const STRATEGY_CANDIDATES: StrategyCandidate[] = [
  {
    ...
    rationale: "Build rapport through attentive engagement",
    baseScore: 0.7,
    tags: ["rapport", "trust"],
  },
  {
    strategy: "Open-Ended Questions",
    rationale: "Encourage elaboration and information sharing",
    baseScore: 0.65,
    tags: ["information", "engagement"],
  },
  {
    strategy: "Empathetic Acknowledgment",
    rationale: "Validate the speaker's perspective",
    baseScore: 0.6,
    tags: ["trust", "rapport"],
  },
  {
    strategy: "Reframing",
    rationale: "Present the situation from a different angle",
    baseScore: 0.6,
    tags: ["persuasion", "negotiation"],
  },
  {
    strategy: "Common Ground",
    rationale: "Identify shared interests to build alignment",
    baseScore: 0.65,
    tags: ["cooperation", "trust"],
  },
  {
    strategy: "Gradual Commitment",
    rationale: "Secure small agreements to build momentum",
    baseScore: 0.55,
    tags: ["negotiation", "persuasion"],
  },
  {
    strategy: "Mirroring",
    rationale: "Reflect language and tone to build connection",
    baseScore: 0.6,
    tags: ["rapport", "trust"],
  },
  {
    strategy: "Strategic Pause",
    rationale: "Allow silence to encourage the other party to fill the gap",
    baseScore: 0.5,
    tags: ["negotiation", "information"],
  },
  {
    strategy: "Anchoring",
    rationale: "Set a reference point to influence the negotiation range",
    baseScore: 0.6,
    tags: ["negotiation", "persuasion"],
  },
  {
    strategy: "De-escalation",
    rationale: "Reduce tension to create a more productive dialogue",
    baseScore: 0.75,
    tags: ["trust", "cooperation"],
  },
  {
    strategy: "Socratic Questioning",
    rationale: "Guide reasoning through targeted questions",
    baseScore: 0.6,
    tags: ["information", "persuasion"],
  },
  {
    strategy: "Concession Strategy",
    rationale: "Offer calculated concessions to advance negotiations",
    baseScore: 0.55,
    tags: ["negotiation", "cooperation"],
  },
];

function buildRLWeights(
  trustLevel: number,
...
function scoreCandidate(
  ...
  if (
    weights.persuasionWeight > 0.35 &&
    candidate.tags.includes("persuasion")
  ) {
    score += 0.08;
  }
  ...
  if (dominantEmotion === "anger" || dominantEmotion === "fear") {
    if (
      candidate.tags.includes("rapport") ||
      candidate.strategy === "De-escalation"
    ) {
      score += 0.12;
    }
  }
  ...
  if (
    dominantIntent === "negotiating" &&
    candidate.tags.includes("negotiation")
  ) {
    score += 0.1;
  }
  if (
    dominantIntent === "threatening" &&
    candidate.strategy === "De-escalation"
  ) {
    score += 0.15;
  }
  if (
    dominantIntent === "cooperating" &&
    candidate.tags.includes("cooperation")
  ) {
    score += 0.1;
  }
  if (
    dominantIntent === "information-seeking" &&
    candidate.tags.includes("information")
  ) {
    score += 0.08;
  }
...
export function generateLlamaRLStrategies(
  ...
  for (const e of window) {
    for (const em of e.emotions) {
      emotionCounts[em.emotionType] =
        (emotionCounts[em.emotionType] ?? 0) + em.confidence;
    }
    for (const i of e.intents) {
      intentCounts[i.intentType] =
        (intentCounts[i.intentType] ?? 0) + i.confidence;
    }
  }
