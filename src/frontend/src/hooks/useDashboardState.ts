...
// ── All AI/ML inference now routes through the service layer ──────────────────
import {
  runEmotionIntent,
  runEthics,
  runStrategyEngine,
...
export interface BeliefState {
  ...
  concerns: string[];
  speakerStates: Record<
    SpeakerRole,
    { entryCount: number; dominantEmotion: string }
  >;
}
...
function updateBeliefState(
  ...
  let trustDelta = 0;
  if (topIntent === "cooperating" || topIntent === "acknowledging")
    trustDelta = 2;
  if (topIntent === "threatening" || topIntent === "deflecting")
    trustDelta = -3;
  ...
  const newPersuasion = Math.max(
    0,
    Math.min(100, prev.persuasionScore + persuasionDelta),
  );
...
export function useDashboardState() {
  ...
  const [beliefState, setBeliefState] =
    useState<BeliefState>(initialBeliefState);
  ...
  const addTranscriptEntry = useCallback(
    async (
      ...
      providedDetectedLanguage?: string,
      ...
      // ── Route all inference through the service layer ──────────────────────
      const [ethicsResult, emotionIntentResult] = await Promise.all([
        runEthics({ text: analysisText }),
        runEmotionIntent({ text: analysisText, detectedLanguage }),
      ]);

      // Convert string arrays to typed objects
      const emotions: EmotionScore[] = emotionIntentResult.emotions.map(
        (e) => ({
          emotionType: e,
      ...
      const toxicityFlags: ToxicityFlag[] = (ethicsResult.flags ?? []).map(
        (f) => ({
          flagType: f,
          confidence: 0.8,
        }),
      ...
      // Generate strategies via the service layer
      const topIntentStr = intents[0]?.intentType ?? "acknowledging";
      const rawStrategies = await runStrategyEngine({
        text: analysisText,
        intents: [topIntentStr],
        trustLevel: beliefState.trustLevel,
        persuasionScore: beliefState.persuasionScore,
      });
  ...
  const toggleTranslation = useCallback((entryIndex: number) => {
    setTranscriptEntries((prev) =>
      prev.map((entry, idx) =>
        idx === entryIndex
          ? { ...entry, showTranslation: !entry.showTranslation }
          : entry,
      ),
    );
  }, []);
  ...
  const translateEntry = useCallback(
    (entryIndex: number, targetLanguage: string) => {
      setTranscriptEntries((prev) =>
        prev.map((entry, idx) => {
